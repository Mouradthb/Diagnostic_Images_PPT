...
export function ResultCard({ item, onRetry, disabled }: ResultCardProps) {
  ...
  return (
    <div
      id={`result-${item.id}`}
      className="neumo-result-card flex min-w-0 flex-col overflow-hidden rounded-2xl border border-[#dce6e8] bg-white shadow-[0_3px_14px_rgba(19,54,65,0.04)] md:flex-row"
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center p-4 sm:p-5">
        {item.status === 'analyzing' && (
          <div className="space-y-3 py-2" role="status">
            <span className="neumo-chip neumo-chip-success inline-flex items-center gap-2 rounded-full bg-[#e7f4f1] px-3 py-1.5 text-xs font-semibold text-[#087f74]"><Loader2 className="size-3.5 animate-spin" /> Analyse en cours</span>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {onRetry && (
                <button
                  ...
                  disabled={disabled}
                  className="neumo-button-secondary inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-[#dce6e8] bg-white px-3 text-xs font-semibold text-[#19313b] transition-colors hover:bg-[#f2f8f7] focus-visible:outline-2 focus-visible:outline-[#087f74] disabled:opacity-50"
                >
        ...
        {item.status === 'completed' && item.result && badgeStyle && (
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="neumo-chip neumo-chip-neutral inline-flex items-center gap-1 rounded-full bg-[#edf1fc] px-2.5 py-1 text-[11px] font-medium text-[#3356c9]"><Info className="size-3.5" /> {statusLabel}</span>
              <span className="neumo-chip rounded-full border border-[#dce6e8] px-2.5 py-1 text-[11px] font-medium text-[#536b75]">Confiance : {item.result.confiance}</span>
            </div>

            <div className="neumo-panel-divider flex flex-wrap gap-x-4 gap-y-1 border-b border-[#e5ecee] pb-3 text-xs text-[#627781]">
            ...
            <div>
              ...
              <p className="neumo-content-inset whitespace-pre-line break-words rounded-xl border border-[#dce6e8] bg-[#f6f9fa] p-3 text-sm leading-relaxed text-[#263e48] sm:p-3.5">{item.result.constat}</p>
            </div>
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <div className="neumo-action-card rounded-xl border border-amber-200/70 bg-amber-50/70 p-3">
              ...
              <div className="neumo-content-inset rounded-xl border border-[#dce6e8] bg-[#f6f9fa] p-3">
